...
def color_video_dataset_process_handler(
    dataset:DatasetHandler,
    frame_segment_length:int=30,
    *denoising_functions
):
    '''
    ...
    for file_name in outVideo_writer.keys():
        ...
        for frame_number in range(1, len(inVideo[file_name].keys())+1):
            ...
            if frame_number % frame_segment_length == 0:
                procs.append(
                    mp.Process(
                        target=video_multi_frame_processing, 
                        args=(task_number, q, denoising_functions, frame_data_list)
                ))
                frame_data_list = []
        procs.append(
            mp.Process(
                target=video_multi_frame_processing, 
                args=(task_number+1, q, denoising_functions, frame_data_list)
        ))
